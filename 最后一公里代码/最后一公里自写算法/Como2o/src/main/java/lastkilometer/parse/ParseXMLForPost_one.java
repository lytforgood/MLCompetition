...
public class ParseXMLForPost_one extends BasicClass{

	public static List<String> assignedOrder_id=new ArrayList<>();//保存已经存储过的订单
	public int number=0;
	/**
	 * 处理第二次配送结果，生成提交格式
	 * 
					...
					for(Entry<String, Map<String,int[]>> entry:three.entrySet()){
						...
						for(Entry<String, int[]> e:four.entrySet()){
							...
							if("pickup".equals(pickupORdeliver)){
								...
								save("results_cache/", "post_"+iteration,Courier_id+","+Addr+","+time[0]+","+time[1]+","+Num+","+Order_id);
...
//	 * 
//	 * */
//	 Map<Integer,List<Map<String,Map<String,int[]>>>> Parse(String pathAndName) throws DocumentException{
//		
//		//Map<int,List<Map<Order_id,Map<pickupORdeliver,int[]>>>> int[]--到达时间和结结束时间
//		Map<Integer,List<Map<String,Map<String,int[]>>>> parseResults=new TreeMap<>();
//		
//		// 创建saxReader对象
//		SAXReader reader = new SAXReader();
//		// 通过read方法读取一个文件 转换成Document对象
...
//			for (int i = 0; i < routes.elements().size(); i++) {
//				
//				List<Map<String,Map<String,int[]>>> one=new ArrayList<>();
//				
//				// 获取第一次路线route
//				Element route = (Element) routes.elements().get(i);
//				// 获取当前送货顺序
//				Iterator<?> eleIt = route.elementIterator();
//				while (eleIt.hasNext()) {
//					//Map<Order_id,Map<pickupORdeliver,int[]>>
...
//	}
	  ...
	  * 获取未配送的O2O订单id
	  * */
	 public List<String> getUnassignedO2OOrder2(String pathAndName) throws DocumentException{
