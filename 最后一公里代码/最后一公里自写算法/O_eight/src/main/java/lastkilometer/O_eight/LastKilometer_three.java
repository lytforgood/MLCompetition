...
import lastkilometer.readCSV.ReadSite_idFromShop_id;
...
//			VehicleRoutingAlgorithm algorithm = Jsprit.createAlgorithm(problem);
			VehicleRoutingAlgorithm algorithm =VehicleRoutingAlgorithms.readAndCreateAlgorithm(problem,"input/algorithmConfig.xml");//algorithmConfig.xml algorithmConfig_greedyWithRegret.xml
...
//					
//			    	int dis_time=calculateDistance(Lng_Lat_Site[0], Lng_Lat_Site[1], Lng_Lat_Spot[0], Lng_Lat_Spot[1]);
...
//							.setPickupTimeWindow(TimeWindow.newInstance(0, 12*60-Math.round(3*Math.sqrt(Num)+5)-dis_time))
//							.setDeliveryTimeWindow(TimeWindow.newInstance(dis_time, 12*60-Math.round(3*Math.sqrt(Num)+5)))
...
//			 vehicleBuilder.setReturnToDepot(false);//设置车辆不返回
	...
	private List<VehicleRoute> setInitialVehicleRoute(VehicleRoutingProblem.Builder vrpBuilder) throws IOException{
		...
		ReadSite_idFromShop_id readSFS=new ReadSite_idFromShop_id();
		Map<String, String> sfs=readSFS.readSite_idFromShop_id();
		
		//读取第一次配送的结果数据
		ReadResults_cache read=new ReadResults_cache();
		Map<String, List<List<List<Object>>>> post_res=read.readResultsFile();
        for(Entry<String, List<List<List<Object>>>> entry:post_res.entrySet()){
        	List<List<List<Object>>> list_one=entry.getValue();
        	
        	//判断当前路径属于哪个网点
			String Addr=null;
			for(List<Object> list_three_C:list_one.get(0)){
				Addr=(String) list_three_C.get(0);
				if(!"A".equals(Addr.substring(0,1))){
					//根据Shop_id判断该路径属于哪个网点
					Addr=sfs.get(Addr);
				}
				break;
			}
			//设置车辆
			Vehicle vehicle=getVehicle(Addr, vrpBuilder);
			if(vehicle==null){
				continue;
			}
			jsprit.core.problem.solution.route.VehicleRoute.Builder vehicleRouteBuilder = VehicleRoute.Builder.newInstance(vehicle);
			//开始设置pickup和Delivery
			for(List<List<Object>> list_two:list_one){
				for(List<Object> list_three:list_two){
					String Order_id=(String) list_three.get(4);
					int Amount=(int) list_three.get(3);
					//根据订单id读取订单信息
					List<Object> list_ofo=ofo.get(Order_id);
					if(list_ofo.size()==3){//电商的订单
						String Spot_id=(String) list_ofo.get(0);
						String Site_id=(String) list_ofo.get(1);
						int Num=(int) list_ofo.get(2);
						//网点的经纬度
						double[] Lng_Lat_Site=Site_Lng_Lat.get(Site_id);
						//配送点的经纬度
						double[] Lng_Lat_Spot=Spot_Lng_Lat.get(Spot_id);
						...
						String Spot_id=(String) list_ofo.get(0);
						String Shop_id=(String) list_ofo.get(1);
						int Pickup_time=(int) list_ofo.get(2);
						int Delivery_time=(int) list_ofo.get(3);
						int Num=(int) list_ofo.get(4);
						//商户的经纬度
						double[] Lng_Lat_Shop=Shop_Lng_Lat.get(Shop_id);
						//配送点的经纬度
						double[] Lng_Lat_Spot=Spot_Lng_Lat.get(Spot_id);
