...
import lastkilometer.readCSV.NeedAssignedOrder_id;
...
public class LastKilometer_two extends ReadData{
	...
	//读取需要配送的订单id
	NeedAssignedOrder_id NAO=new NeedAssignedOrder_id();
...
//			VehicleRoutingAlgorithm algorithm = Jsprit.createAlgorithm(problem);
			VehicleRoutingAlgorithm algorithm =VehicleRoutingAlgorithms.readAndCreateAlgorithm(problem,"input/algorithmConfig_greedyWithRegret.xml");
				...
				for (VehicleRoute route : solution.getRoutes()) {
					...
					for (TourActivity act : route.getTourActivities().getActivities()) {
						...
						if ("pickupShipment".equals(pickupORdeliver)) {
							...
							save("results/", "post_last_2", Courier_id + "," + Addr + "," + (int)act.getArrTime() + "," +  (int)act.getEndTime()
									+ "," + Num + "," + Order_id);
	...
	private List<Shipment> setOrder(List<String> Site_id_list,int iteration) throws NumberFormatException, IOException{
		...
		//读取已经配送的订单id
		List<String> needAssignedOrder_id=NAO.readNeedAssignedOrder_id();
